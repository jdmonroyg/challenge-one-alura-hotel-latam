...
public class HuespedDAO {
    ...
    public void modificar(Huesped huesped) {
        String query = "update huespedes " +
                "set nombre=?, apellido=?, fechaNacimiento=?, nacionalidad=?, telefono=? " +
                "where idhuespedes=?";
        try (con;
             PreparedStatement pstm = con.prepareStatement(query)) {
    ...
    /*public void eliminar(int id) {
        String query="Delete from huespedes where idhuespedes=?";
        try (con;
            PreparedStatement pstm=con.prepareStatement(query)){
            pstm.setInt(1,id);
            pstm.execute();
    ...
    private void agregarHuespedList(List<Huesped> huespedList, PreparedStatement preparedStatement) throws SQLException {
